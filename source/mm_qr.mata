*! version 1.0.0  26mar2021  Ben Jann
version 11
mata:
...
real colvector mm_qr::unzero(real colvector y, real matrix X, real colvector b)
{
    ...
    if (offset>=.) offset = shift // can happen in case of perfect fit
    ...
    // fallback rule: move critical residuals away from zero by a fixed
    // amount; do not update coefficients; this may lead to somewhat biased
    // estimates; a better approach would be to return to the above approach
    // and keep on searching for suitable starting values
    r = y - _xb(X, b)
    return(r - shift * (r:>-eps :& r:<0) + shift * (r:>=0 :& r:<eps))
}
